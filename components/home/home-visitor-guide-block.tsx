...
const bullets: { label: string; icon: typeof MapPinned }[] = [
  { label: "Map & trail", icon: MapPinned },
  { label: "Swimming holes", icon: Droplets },
    ...
    >
      ...
      <ul className="mt-12 grid gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
        {bullets.map(({ label, icon: Icon }) => (
          <li key={label}>
            <div className="flex h-full items-center gap-4 rounded-2xl border border-[#E2E0D8] bg-white p-6 shadow-sm transition duration-300 ease-out hover:-translate-y-0.5 hover:shadow-md">
